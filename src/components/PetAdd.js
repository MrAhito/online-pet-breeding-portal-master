...
class PetAdd extends Component {
    ...
    handleSlt(e, a) {
        ...
        } else if (e.target.value === "" && a === 'vac') {
            this.setState({ vacPos: false });
        }
    }
    handlegend(a) {
        if (a === 'gend') {
    ...
    async handleSubmit() {
        this.setState({ progPos: true });
        const a = await this.fileUpload(this.state.anti);
        const b = await this.fileUpload(this.state.dworm);
        const c = this.state.petVac;
        const d = await this.fileUpload(this.state.vacs);
        const e = await this.fileUpload(this.state.check);
        const f = await this.fileUpload(this.state.vit);
        createPet(this.state.userBdate,
            this.state.petName,
            this.state.petGend,
            this.state.petSpec,
            this.state.petBreed,
                    ...
                    <div className='regForm-wrapper'>
                        <div className='col1 petAddDiv' >
                            <div className="field col2a">
                                <input type="text" name='pName' id='pName' onChange={(e) => { this.setState({ petName: e.target.value }) }} className='login' placeholder="Pet Name" required />
                            ...
                            <div className="field col2a">
                                <select id='regGend' name='regGend' onClick={() => { this.handlegend('gend') }} onBlur={(e) => { this.handleSlt(e, 'gend') }} className={this.state.gendPos ? 'regSlt gendAct' : 'regSlt '} defaultValue='' onChange={(e) => { this.sltChange(e, 'gend') }} required>
                            ...
                            <div className="field col2b">
                                <select id='regFur' name='regFur' onClick={() => { this.handlegend('fur') }} onBlur={(e) => { this.handleSlt(e, 'fur') }} className={this.state.furPos ? 'regSlt gendAct' : 'regSlt '} defaultValue='' onChange={(e) => { this.sltChange(e, 'fur') }} required>
                            ...
                            <div className="field col2a">
                                <select id='pSpec' name='pSpec' onClick={() => { this.handlegend('spec') }} onBlur={(e) => { this.handleSlt(e, 'spec') }} className={this.state.specPos ? 'regSlt gendAct' : 'regSlt '} defaultValue='' onChange={(e) => { this.sltChange(e, 'spec') }} required>
                                    <option value='' disabled></option>
                            ...
                            <div className="field col2b">
                                <select id='regEye' name='regEye' onClick={() => { this.handlegend('eye') }} onBlur={(e) => { this.handleSlt(e, 'eye') }} className={this.state.eyePos ? 'regSlt gendAct' : 'regSlt '} defaultValue='' onChange={(e) => { this.sltChange(e, 'eye') }} required>
                                    <option value='' disabled></option>
                            ...
                            <div className="field col2a">
                                <select id='pBreed' name='pBreed' onClick={() => { this.handlegend('breed') }} onBlur={(e) => { this.handleSlt(e, 'breed') }} className={this.state.breedPos ? 'regSlt gendAct' : 'regSlt '} defaultValue='' onChange={(e) => { this.sltChange(e, 'breed') }} required>
                                    <option value='' disabled></option>
                            ...
                            <div className="field col2b">
                                <input type="text" name='pDesc' id='pDesc' placeholder=' ' onChange={(e) => { this.setState({ petDesc: e.target.value }) }} className='login' />
                                <label id='LRPDesc' className='label-log' htmlFor='pDesc'>Descripion</label>
                            </div>

                        </div>
                        <div className='col1 petAddDiv healthHistory' >
                            <div className='health-wrapper'>
                                <h2 className='health-header'>Health History</h2>
                                <div className='pet-div'></div>
                                <div className='health-input' >
                                    <div className="field health-inputa">
                                        <select id='regGend' name='regGend' onClick={() => { this.handlegend('vac') }} onBlur={(e) => { this.handleSlt(e, 'vac') }} className={this.state.vacPos ? 'regSlt gendAct' : 'regSlt '} defaultValue='' onChange={(e) => { this.sltChange(e, 'vac') }} required>
                                            <option value='' hidden></option>
                                            <option value='2 - in - 1'>2 - in - 1</option>
                                            <option value='5 - in - 1'>5 - in - 1</option>
                                            <option value='5 - in - 1(LR)'>5 - in - 1(LR)</option>
                                            <option value='6 - in - 1'>6 - in - 1</option>
                                            <option value='8 - in - 1'>8 - in - 1</option>
                                        </select>
                                        <label id='LRgend' className={this.state.vacPos ? 'label-log gendMove' : 'label-log gend'} htmlFor="regGend">Vaccine - Set</label>
                                    </div>

                                    <input disabled={this.state.petVac ? false : true} type='file' accept="application/pdf,application/vnd.ms-excel" id='fivein1' onChange={(e) => { this.setState({ vacs: e.target.files[0] }) }} />
                                </div>
                                <div className='health-input' >
                                    <label>Anti-Rabbies:</label>
                                    <input type='file' accept="application/pdf,application/vnd.ms-excel" id='anti-rabbies' onChange={(e) => { this.setState({ anti: e.target.files[0] }) }} />
                                </div>
                                <div className='health-input' >
                                    <label>Deworming:</label>
                                    <input type='file' accept="application/pdf,application/vnd.ms-excel" id='deworming' onChange={(e) => { this.setState({ dworm: e.target.files[0] }) }} />
                                </div>
                                <div className='health-input' >
                                    <label>Check Ups:</label>
                                    <input type='file' accept="application/pdf,application/vnd.ms-excel" id='checkup' onChange={(e) => { this.setState({ check: e.target.files[0] }) }} />
                                </div>
                                <div className='health-input' >
                                    <label>Vitamins:</label>
                                    <input type='file' accept="application/pdf,application/vnd.ms-excel" id='Vitamins' onChange={(e) => { this.setState({ vit: e.target.files[0] }) }} />
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='btnAddPEts'>
                        <button type={this.state.userBdate !== '' && this.state.petName !== '' && this.state.petGend !== '' && this.state.petFur !== '' && this.state.petBreed !== '' && this.state.petEye !== '' && this.state.petSpec !== '' ? 'button' : 'submit'} onClick={() => { this.state.userBdate !== '' && this.state.petName !== '' && this.state.petGend !== '' && this.state.petFur !== '' && this.state.petBreed !== '' && this.state.petEye !== '' && this.state.petSpec !== '' && this.handleSubmit(); }} className='btn btn-log'>Add</button>
